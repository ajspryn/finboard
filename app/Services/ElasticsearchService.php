...
class ElasticsearchService
{
    ...
    public function search(string $indexName, array $query, int $from = 0, int $size = 10): array
    {
        try {
            ...
            // Also fix nested match_all queries in bool queries
            if (isset($query['bool'])) {
                if (isset($query['bool']['must'])) {
                    foreach ($query['bool']['must'] as &$mustQuery) {
                        if (isset($mustQuery['match_all']) && $mustQuery['match_all'] === []) {
                            $mustQuery['match_all'] = new \stdClass();
                        }
                    }
                }
                if (isset($query['bool']['should'])) {
                    foreach ($query['bool']['should'] as &$shouldQuery) {
                        if (isset($shouldQuery['match_all']) && $shouldQuery['match_all'] === []) {
                            $shouldQuery['match_all'] = new \stdClass();
                        }
                    }
                }
            }
    ...
    public function countDocuments(string $indexName, array $query = []): int
    {
        try {
            ...
            Log::error("Failed to count documents in index: {$indexName}", [
                'error' => $e->getMessage(),
            ]);
